...
if ($action == 'create') {
  if (!empty($iduser_auteur) && !empty($iduser_cible) && !empty($idtache) && !empty($note) && !empty($commentaires)) {
    $avisdb->create($iduser_auteur, $iduser_cible, $idtache,$intitule, $note, $commentaires);
...
if ($action == 'update') {
  ...
  if (!empty($iduser_auteur) && !empty($iduser_cible) && !empty($idtache) && !empty($note) && !empty($commentaires)) {
    $avisdb->update($id, $iduser_auteur, $iduser_cible, $idtache, $intitule, $note, $commentaires);
    $_SESSION['erreur'] = array(
      'type' => 'success',
